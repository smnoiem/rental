<?php
    ...
    if(mysqli_num_rows($adsR)!=1 || $adsRow['users_id']!=$users_id) echo("This ad is not associated with this users.");
        ...
        if(isset($_FILES['img_file'])&&($_FILES['img_file']['error']<=0)){
            ...
            location='$location', details='$description', image='$imageName' WHERE users_id=$users_id";

        }
        else{

            $updateSql = "UPDATE ads SET title='$name', category='$cat', sub_category='$subcat',
            location='$location', details='$description' WHERE users_id=$users_id";
        }
